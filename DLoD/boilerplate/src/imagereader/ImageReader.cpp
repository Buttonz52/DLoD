...
bool InitializeTexture(MyTexture *texture, const string &imageFileName)
{
    ...
    // check if we're loading an image in HDR format
    bool hdr = (imageFileName.length() - imageFileName.rfind(".hdr") == 4);

    // use stb_image to read the file
    int width, height, numComponents;
    void *data = 0;
    if (hdr)
        data = stbi_loadf(imageFileName.c_str(), &width, &height, &numComponents, 0);
    else
        data = stbi_load(imageFileName.c_str(), &width, &height, &numComponents, 0);
    ...
    GLenum format = (numComponents == 3) ? GL_RGB : GL_RGBA;
	GLint internalFormat = (numComponents ==3) ? GL_SRGB8 : GL_SRGB8_ALPHA8;
    GLint hdrFormat = (numComponents == 3) ? GL_RGB16F : GL_RGBA16F;
    if (hdr) {
        glTexImage2D(texture->textureType, 0, hdrFormat,
                     texture->width, texture->height,
                     0, format, GL_FLOAT, data);
    }
    else {
        glTexImage2D(texture->textureType, 0, internalFormat,
                     texture->width, texture->height,
...
bool InitializeCubeMap(MyTexture *texture, const string &imageFileName)
{
    // ask stb_image to organize the data buffer in OpenGL axes convention
    stbi_set_flip_vertically_on_load(false);

    // check if we're loading an HDR image
    bool hdr = (imageFileName.length() - imageFileName.rfind(".hdr") == 4);
    ...
    // quick check for invalid cube map size or configuration
    if (width / 4 != height / 3)
        return false;
    int unit = width / 4;

    // offset values for each of the 6 faces of the packed cube map,
    // assuming horizontal cross layout
    GLenum targets[6] = {
        GL_TEXTURE_CUBE_MAP_NEGATIVE_X, GL_TEXTURE_CUBE_MAP_POSITIVE_X,
        GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, GL_TEXTURE_CUBE_MAP_POSITIVE_Y,
        GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, GL_TEXTURE_CUBE_MAP_POSITIVE_Z
    };
    int rowSkip[6] = { unit, unit, 2*unit, 0, unit, unit };
    int pixelSkip[6] = { 0, 2*unit, unit, unit, 3*unit, unit };
    ...
    for (int f = 0; f < 6; ++f)
    {
        glPixelStorei(GL_UNPACK_SKIP_ROWS, rowSkip[f]);
        glPixelStorei(GL_UNPACK_SKIP_PIXELS, pixelSkip[f]);

        if (hdr) {
            glTexImage2D(targets[f], 0, hdrFormat, unit, unit,
                         0, format, GL_FLOAT, data);
        }
        else {
            glTexImage2D(targets[f], 0, format, unit, unit,
                         0, format, GL_UNSIGNED_BYTE, data);
        }
    }

    // restore pixel unpacking defaults
